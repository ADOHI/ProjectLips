...
public class StylizedSurfaceEditor : BaseShaderGUI {
        ...
        foreach (MaterialProperty property in properties) {
            ...
            if (!hideInInspector && !skipProperty) {
                ...
                if (EditorGUI.EndChangeCheck()) {
                    if (property.name.Contains("_OutlineEnabled")) {
                        ToggleOutlineRenderer(_target);
                    }
                }
                */
            }
    ...
    private static void ToggleOutlineRenderer(Material target) {
        ...
        var outlineEnabled = target.IsKeywordEnabled("DR_OUTLINE_ON");
        foreach (var feature in features) {
            if (feature.GetType().Name.Contains("Flat Kit Per-Object Outline")) {
                feature.SetActive(outlineEnabled);
                var m = "<b>[Flat Kit]</b> Per-object outlines are now " +
                        $"<i>{(outlineEnabled ? "enabled" : "disabled")}</i>.";
                Debug.Log(m);
            }
        }
    }
